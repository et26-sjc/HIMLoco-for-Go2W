"""Zero-shot quiet evaluation of A0.1 gated axial admittance.

The neural policy is loaded from the already-trained ``MC100HzMinimal`` run.
Only the low-level controller changes to the impact-gated, rate-limited axial
admittance variant.
"""
...
# Reuse the shared 100 Hz evaluator without duplicating the protocol.  It always
# asks for quiet_mc_100hz_v2 internally, so redirect that key to A0.1 here.
